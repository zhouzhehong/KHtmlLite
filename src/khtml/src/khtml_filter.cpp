...
// star, so we escape the rest before passing to QRegularExpression.
...
static QRegularExpression fromAdBlockWildcard(const QString &wcStr)
{
    ...
    // Convert wildcard pattern (*) to regular expression using Qt's built-in converter
    return QRegularExpression(QRegularExpression::wildcardToRegularExpression(out));
}
...
bool FilterSet::isUrlMatched(const QString &url)
{
    ...
    for (int c = 0; c < reFilters.size(); ++c) {
        if (reFilters[c].match(url).hasMatch()) {
            return true;
        }
    }
...
void StringsMatcher::addWildedString(const QString &prefix, const QRegularExpression &rx)
{
                ...
                if (index >= 0) {
                    ...
                    if (k - 8 + flen < len && rePrefixes[index] == str.midRef(k - 7, flen)) {
                        ...
                        QString remainder = QString::fromRawData(str.unicode() + remStart,
                                            str.length() - remStart);
                        if (reFilters[index].match(remainder, 0, QRegularExpression::NormalMatch, QRegularExpression::AnchoredMatchOption).hasMatch()) {
                            if (by != nullptr) {
