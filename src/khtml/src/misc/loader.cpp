...
void CachedFont::data(QBuffer &buffer, bool eof)
{
    ...
    // some fonts are compressed.
    {
        ...
        std::unique_ptr<KCompressionDevice> dev(new KCompressionDevice(&buffer, false /*autoDeleteInDevice*/, compressionType));
...
{
    int res = 0;
    for (Request *req : m_requestsLoading)
...
//         assert(co->canDelete());
    ...
    for (CachedObject *co : *freeList) {
        if (!co->canDelete()) {
            ...
            crash = true;
            /*
            for (CachedObjectClient* cur : (*co->m_clients)))
            {
