...
void KHTMLParser::parseToken(Token *t)
{
    ...
    bool inSvgNamespace = (m_svgDepth > 0 || t->tid == ID_SVG) && m_foreignObjectDepth == 0;
    bool isSvgElem = (t->tid != ID_TEXT) && inSvgNamespace;
    if (t->tid == ID_SVG && m_foreignObjectDepth == 0) {
        m_svgDepth++;
    }
    if (t->tid == ID_FOREIGNOBJECT && m_svgDepth > 0) {
        m_foreignObjectDepth++;
    }
    ...
    // SVG elements: created with correct SVG type in getElement(),
    // then inserted through the normal HTML parser path below.
    // This avoids bypassing blockStack which caused heap corruption.
...
NodeImpl *KHTMLParser::getElement(Token *t)
{
    ...
    if (t->tid != ID_TEXT && m_foreignObjectDepth == 0 && (m_svgDepth > 0 || t->tid == ID_SVG)) {
        DOMString tagName = LocalName::fromId(t->tid).toString();
        n = document->createElementNS(SVG_NAMESPACE, tagName);
        if (n) {
            return n;
        }
    }
...
{
    // Track SVG nesting depth for getElement() namespace detection.
    // Actual close-tag handling goes through the normal HTML path below.
    if (t->tid == ID_SVG + ID_CLOSE_TAG && m_svgDepth > 0) {
        m_svgDepth--;
        // SVG elements are pushed with n->id() == tagId + ID_CLOSE_TAG
        // (e.g. ID_RECT+ID_CLOSE_TAG), so popBlock(ID_SVG) below cannot match
        // them and the parser would stay stuck inside the SVG subtree, causing
        // all following elements to be nested into the last SVG child.
        // Pop every SVG-namespace block (any id beyond ID_LAST_TAG) up to and
        // including the <svg> element itself.
        while (blockStack) {
            int sid = blockStack->id;
            if (sid == ID_SVG || sid == ID_SVG + ID_CLOSE_TAG) {
                popOneBlock();
                break;
            }
            if (sid > ID_LAST_TAG) {
    ...
    if (t->tid == ID_FOREIGNOBJECT + ID_CLOSE_TAG && m_foreignObjectDepth > 0) {
        m_foreignObjectDepth--;
        while (blockStack) {
            int sid = blockStack->id;
            if (sid == ID_FOREIGNOBJECT || sid == ID_FOREIGNOBJECT + ID_CLOSE_TAG) {
                popOneBlock();
                break;
            }
            if (sid > ID_LAST_TAG) {
