...
#include "platform/graphics/AffineTransform.h"
...
void RenderSVGRoot::paint(PaintInfo &paintInfo, int parentX, int parentY)
{
    ...
    // SVG child renderers use absolute coordinates and ignore parentX/parentY.
    // Translate the painter so SVG content lands inside this element's box.
    childPaintInfo.p->translate(m_x + parentX, m_y + parentY);

    // Apply viewBox-to-viewport scaling so content with a viewBox is mapped
    // into the element's CSS box. Without this, viewBox coordinates are
    // rendered 1:1 and overflow the viewport. Use the computed viewport
    // (which falls back to CSS size / 300x150 default) rather than the CSS
    // box, which may still be unset for width/height-less inline <svg>.
    if (SVGSVGElement *svg = static_cast<SVGSVGElement *>(element())) {
        AffineTransform vb = svg->viewBoxToViewTransform(viewport().width(), viewport().height());
        if (!vb.isIdentity()) {
            QTransform vt(vb.a(), vb.b(), vb.c(), vb.d(), vb.e(), vb.f());
            childPaintInfo.p->setTransform(vt, true);
        }
    }
...
void RenderSVGRoot::calcViewport()
{
    ...
    if (svgelem->hasTagName(SVGNames::svgTag)) {
        ...
        } else if (width.unitType() == LengthTypePercentage) {
            // Inline <svg> without explicit width/height attributes defaults to
            // "100%", which is meaningless without a sized viewport container.
            // Fall back to the CSS computed size, then to the CSS
            // replaced-element default (300x150) used by modern browsers.
            if (style() && style()->width().type() == Fixed) {
                w = style()->width().value();
            } else {
                w = 300.0f;
            }
        ...
        } else if (height.unitType() == LengthTypePercentage) {
            if (style() && style()->height().type() == Fixed) {
