...
void Window::mark()
{
    ...
    for (DelayedAction *action : m_delayed) {
        action->mark();
    }
}
...
void Window::afterScriptExecution()
{
    ...
    m_delayed.clear();
    for (DelayedAction *act : delayedActions) {
        if (!act->execute(this)) {
...
JSValue *Window::executeOpenWindow(ExecState *exec, const QUrl &url, const QString &frameName, const QString &features)
{
    ...
    static const QRegularExpression m(QStringLiteral(",|\\b\\s+(?!=)"));
...
void Window::showSuppressedWindows()
{
    ...
    m_suppressedWindowInfo.clear();
    for (const SuppressedWindowInfo &info : suppressedWindowInfo) {
...
void WindowQObject::timerEvent(QTimerEvent *)
{
    ...
    for (ScheduledAction *action : toExecute) {
