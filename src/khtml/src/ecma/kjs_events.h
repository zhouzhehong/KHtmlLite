...
public:
    ...
    // Raw handler source (e.g. "foo(event)" from onclick="..."). Retained
    // after parsing so the QuickJS event path can re-execute it.
    const QString &sourceCode() const { return code; }
private:
