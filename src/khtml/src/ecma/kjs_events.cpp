...
void JSEventListener::handleEvent(DOM::Event &evt)
{
    ...
    // QuickJS event dispatch: modern JS callbacks run through QuickJS to
    // avoid KJS heap corruption on complex pages. Inline handlers use their
    // source code; addEventListener callbacks are looked up in the QJS table.
    if (proxy && proxy->quickJSAvailable()) {
        QString handlerSource;
        if (JSLazyEventListener *lazy = dynamic_cast<JSLazyEventListener *>(this)) {
            handlerSource = lazy->sourceCode();
        }
        ...
        if (proxy->dispatchEventToQuickJS(evt.handle(), target, handlerSource)) {
            return;
        }
    }

    // KJS fallback path (used when QuickJS is unavailable or fails).
...
void JSLazyEventListener::parseCode() const
{
    if (!parsed) {
        ...
        // NOTE: code is retained (not cleared) so the QuickJS event path
        // can re-execute inline handlers without going through KJS.
        // code.clear();
