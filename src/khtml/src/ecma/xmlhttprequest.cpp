...
JSValue *XMLHttpRequest::getResponseHeader(const QString &name, int &ec) const
{
    ...
    QRegularExpression headerLinePattern(name + QLatin1Char(':'), QRegularExpression::CaseInsensitiveOption);

    int matchLength = 0;
    int headerLinePos = -1;
    int searchFrom = 0;
    while (true) {
        QRegularExpressionMatch headerMatch = headerLinePattern.match(responseHeaders, searchFrom);
        if (!headerMatch.hasMatch()) {
            headerLinePos = -1;
            break;
        }
        headerLinePos = headerMatch.capturedStart();
        matchLength = headerMatch.capturedLength();
        if (headerLinePos == 0 || responseHeaders[headerLinePos - 1] == QLatin1Char('\n')) {
            break;
        }
        searchFrom = headerLinePos + 1;
    }
...
static QString encodingFromContentType(const QString &type)
{
    ...
    if (index > -1) {
        encoding = type.mid(index + 1).remove(QRegularExpression(QStringLiteral("charset[ ]*=[ ]*"), QRegularExpression::CaseInsensitiveOption)).trimmed();
    }
    return encoding;
}
