...
//     case CSS_PROP_PAGE:                 // <identifier> | auto // ### CHECK
    ...
    case CSS_PROP_DISPLAY:
        ...
        // flex | inline-flex (minimal flexbox support)
        if ((id >= CSS_VAL_INLINE && id <= CSS_VAL_TABLE_CAPTION) || id == CSS_VAL_NONE ||
            id == CSS_VAL_FLEX || id == CSS_VAL_INLINE_FLEX) {
            valid_primitive = true;
        }
        break;
    case CSS_PROP_FLEX:
        // flex: <grow> [<shrink> <basis>] | none — we only honour the grow part.
        if (id == CSS_VAL_NONE || id == CSS_VAL_INHERIT
            || (value && value->unit == CSSPrimitiveValue::CSS_NUMBER)) {
            valid_primitive = true;
        }
        break;
    case CSS_PROP_FLEX_DIRECTION:
        if ((id >= CSS_VAL_ROW && id <= CSS_VAL_COLUMN_REVERSE) || id == CSS_VAL_INHERIT) {
            valid_primitive = true;
        }
        break;
    case CSS_PROP_JUSTIFY_CONTENT:
        if ((id >= CSS_VAL_FLEX_START && id <= CSS_VAL_SPACE_AROUND) || id == CSS_VAL_CENTER) {
            valid_primitive = true;
        }
        break;
    case CSS_PROP_ALIGN_ITEMS:
        if (id == CSS_VAL_STRETCH || id == CSS_VAL_FLEX_START || id == CSS_VAL_FLEX_END
            || id == CSS_VAL_CENTER) {
            valid_primitive = true;
        }
        break;
    ...
    case CSS_PROP_FLEX_WRAP:
        if (id == CSS_VAL_WRAP || id == CSS_VAL_NOWRAP) {
            valid_primitive = true;
        }
        break;
    case CSS_PROP_GAP:
        if (value && (value->unit == CSSPrimitiveValue::CSS_PX
    ...
    if (valid_primitive) {
        ...
        if (id != 0) {
            ...
            parsedValue = new CSSPrimitiveValueImpl(value->fValue,
                                                    (CSSPrimitiveValue::UnitTypes) value->unit);
        } else if (value->unit >= CSSPrimitiveValue::CSS_VH &&
                   value->unit <= CSSPrimitiveValue::CSS_VMAX) {
            parsedValue = new CSSPrimitiveValueImpl(value->fValue,
                                                    (CSSPrimitiveValue::UnitTypes) value->unit);
...
{
    std::unique_ptr<CSSValueListImpl> values(
...
int DOM::CSSParser::lex(void *_yylval)
{
    ...
    case DIMEN: {
#if 0
        // Recognize viewport-relative units (vh/vw/vmin/vmax) here because
        // the prebuilt tokenizer does not have dedicated rules for them.
        const QString dim = QString((QChar *)t, length);
        int unitTok = 0;
        int suffixLen = 0;
        if (dim.endsWith(QLatin1String("vmin"), Qt::CaseInsensitive)) {
            unitTok = VMIN; suffixLen = 4;
        } else if (dim.endsWith(QLatin1String("vmax"), Qt::CaseInsensitive)) {
            unitTok = VMAX; suffixLen = 4;
        } else if (dim.endsWith(QLatin1String("vh"), Qt::CaseInsensitive)) {
            unitTok = VH; suffixLen = 2;
        } else if (dim.endsWith(QLatin1String("vw"), Qt::CaseInsensitive)) {
            unitTok = VW; suffixLen = 2;
        }
        if (0 && unitTok) {
            bool ok = false;
            const double num = dim.left(length - suffixLen).toDouble(&ok);
            if (ok) {
                yylval->val = qMin(num, dIntMax);
                return unitTok;
            }
        }
#endif
