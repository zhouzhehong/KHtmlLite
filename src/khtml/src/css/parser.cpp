...
{
    /* Hardcoded fallback for properties added after gperf codegen */
...
{
    /* Hardcoded values added after gperf codegen */
    if (len == 4 && !strncmp(tagStr, "flex", 4)) return 334;
    if (len == 11 && !strncmp(tagStr, "inline-flex", 11)) return 335;
    if (len == 3 && !strncmp(tagStr, "row", 3)) return 336;
    if (len == 6 && !strncmp(tagStr, "column", 6)) return 337;
    if (len == 11 && !strncmp(tagStr, "row-reverse", 11)) return 338;
    if (len == 14 && !strncmp(tagStr, "column-reverse", 14)) return 339;
    if (len == 10 && !strncmp(tagStr, "flex-start", 10)) return 340;
    if (len == 8 && !strncmp(tagStr, "flex-end", 8)) return 341;
    if (len == 13 && !strncmp(tagStr, "space-between", 13)) return 342;
    if (len == 12 && !strncmp(tagStr, "space-around", 12)) return 343;
    if (len == 7 && !strncmp(tagStr, "stretch", 7)) return 344;
    if (len == 4 && !strncmp(tagStr, "wrap", 4)) return 345;
    { // HTML CSS Values
...
yyreduce:
  ...
  case 162:
    {
        (yyval.value).id = 0;
        ParseString ps = (yyvsp[(1) - (2)].string);
        int unit = CSSPrimitiveValue::CSS_DIMENSION;
        int suffixLen = 0;
        if (ps.length >= 4) {
            unsigned short *s = ps.string;
            int n = ps.length;
            unsigned short c1 = s[n-4] | 0x20;
            unsigned short c2 = s[n-3] | 0x20;
            unsigned short c3 = s[n-2] | 0x20;
            unsigned short c4 = s[n-1] | 0x20;
            if (c1 == 'v' && c2 == 'm' && c3 == 'i' && c4 == 'n') {
                unit = CSSPrimitiveValue::CSS_VMIN; suffixLen = 4;
            } else if (c1 == 'v' && c2 == 'm' && c3 == 'a' && c4 == 'x') {
                unit = CSSPrimitiveValue::CSS_VMAX; suffixLen = 4;
            }
        }
        if (suffixLen == 0 && ps.length >= 2) {
            unsigned short *s = ps.string;
            int n = ps.length;
            unsigned short c1 = s[n-2] | 0x20;
            unsigned short c2 = s[n-1] | 0x20;
            if (c1 == 'v' && c2 == 'h') {
                unit = CSSPrimitiveValue::CSS_VH; suffixLen = 2;
            } else if (c1 == 'v' && c2 == 'w') {
                unit = CSSPrimitiveValue::CSS_VW; suffixLen = 2;
            }
        }
        if (suffixLen > 0) {
            char buf[64];
            int numLen = ps.length - suffixLen;
            if (numLen > 63) numLen = 63;
            for (int i = 0; i < numLen; i++) buf[i] = (char)ps.string[i];
            buf[numLen] = 0;
            (yyval.value).fValue = atof(buf);
        } else {
            (yyval.value).string = ps;
        }
        (yyval.value).unit = unit;
    }
    break;
