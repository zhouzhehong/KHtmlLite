                                   ...
                                   const QUrl &url, bool _strictParsing)
{
    ...
    for (StyleSheetImpl *sh : styleSheets->styleSheets) {
...
void CSSStyleSelector::loadDefaultStyle(const KHTMLSettings *s, DocumentImpl *doc)
{
...

    {
        ...
        QRegularExpression checkVersion(QStringLiteral("KHTML_STYLE_VERSION:\\s*(\\d+)"));
        QRegularExpressionMatch versionMatch = checkVersion.match(style);
        if (!versionMatch.hasMatch() || versionMatch.captured(1).toInt() != KHTML_STYLE_VERSION) {
...
void CSSStyleSelector::adjustRenderStyle(RenderStyle *style, DOM::ElementImpl *e)
{
    ...
    if (style->display() != NONE) {
        ...
        if (style->display() != BLOCK && style->display() != TABLE &&
                style->display() != FLEX && style->display() != INLINE_FLEX &&
...
Length CSSStyleSelector::convertToLength(CSSPrimitiveValueImpl *primitiveValue, RenderStyle *style, RenderStyle *rootStyle, int logicalDpiY, bool *ok)
{
    Length l;
    if (!primitiveValue) {
        if (ok) {
        ...
        if (type > CSSPrimitiveValue::CSS_PERCENTAGE && type < CSSPrimitiveValue::CSS_DEG) {
        ...
        } else if (type >= CSSPrimitiveValue::CSS_VH && type <= CSSPrimitiveValue::CSS_VMAX) {
            int viewportWidth = 0, viewportHeight = 0;
            if (element && element->document() && element->document()->view()) {
                KHTMLView *view = element->document()->view();
                viewportWidth = view->visibleWidth();
                viewportHeight = view->visibleHeight();
            }
            double factor = 1.0;
            switch (type) {
            ...
            case CSSPrimitiveValue::CSS_VMIN:
                factor = qMin(viewportWidth, viewportHeight) / 100.0;
                break;
            case CSSPrimitiveValue::CSS_VMAX:
                factor = qMax(viewportWidth, viewportHeight) / 100.0;
                break;
            }
            l = Length(qRound(primitiveValue->floatValue((CSSPrimitiveValue::UnitTypes)type) * factor), Fixed);
...
// ident only properties
    ...
    case CSS_PROP_DISPLAY: {
        ...
        if (id == CSS_VAL_FLEX) { style->setDisplay(FLEX); break; }
        if (id == CSS_VAL_INLINE_FLEX) { style->setDisplay(INLINE_FLEX); break; }
        ...
        // +inherit
    {
        ...
        if (primitiveValue && !apply) {
            ...
            if (type > CSSPrimitiveValue::CSS_PERCENTAGE && type < CSSPrimitiveValue::CSS_DEG)
                ...
                l = convertToLength(primitiveValue, style, m_rootStyle, logicalDpiY);
            else if (type == CSSPrimitiveValue::CSS_PERCENTAGE) {
                l = Length(primitiveValue->floatValue(CSSPrimitiveValue::CSS_PERCENTAGE), Percent);
            } else if (type == CSSPrimitiveValue::CSS_HTML_RELATIVE) {
                l = Length(int(primitiveValue->floatValue(CSSPrimitiveValue::CSS_HTML_RELATIVE)), Relative);
...
// shorthand properties
    ...
    case CSS_PROP_OPACITY:
        ...
        break;
    case CSS_PROP_FLEX: {
        if (isInitial || isInherit) { style->setFlexGrow(0); return; }
        // "flex: 1", "flex: 2 1 auto" — take the leading number as grow.
        if (!primitiveValue || primitiveValue->primitiveType() != CSSPrimitiveValue::CSS_NUMBER) return;
        style->setFlexGrow(qMax(0.0, primitiveValue->floatValue(CSSPrimitiveValue::CSS_NUMBER)));
        break;
    }
    case CSS_PROP_FLEX_DIRECTION: {
        if (isInitial || isInherit) { style->setFlexDirection(0); return; }
        if (!primitiveValue) return;
        const int fid = primitiveValue->getIdent();
        if (fid == CSS_VAL_ROW) style->setFlexDirection(0);
        else if (fid == CSS_VAL_ROW_REVERSE) style->setFlexDirection(1);
        else if (fid == CSS_VAL_COLUMN) style->setFlexDirection(2);
        else if (fid == CSS_VAL_COLUMN_REVERSE) style->setFlexDirection(3);
        break;
    }
    case CSS_PROP_JUSTIFY_CONTENT: {
        if (isInitial || isInherit) { style->setJustifyContent(0); return; }
        if (!primitiveValue) return;
        const int jid = primitiveValue->getIdent();
        if (jid == CSS_VAL_CENTER) style->setJustifyContent(1);
        else if (jid == CSS_VAL_FLEX_END) style->setJustifyContent(2);
        else if (jid == CSS_VAL_SPACE_BETWEEN) style->setJustifyContent(3);
        else if (jid == CSS_VAL_SPACE_AROUND) style->setJustifyContent(4);
        break;
    }
    case CSS_PROP_ALIGN_ITEMS: {
        if (isInitial || isInherit) { style->setAlignItems(0); return; }
        if (!primitiveValue) return;
        const int aid = primitiveValue->getIdent();
        if (aid == CSS_VAL_STRETCH) style->setAlignItems(0);
        else if (aid == CSS_VAL_FLEX_START) style->setAlignItems(1);
        else if (aid == CSS_VAL_CENTER) style->setAlignItems(2);
        else if (aid == CSS_VAL_FLEX_END) style->setAlignItems(3);
        break;
    }
    ...
    case CSS_PROP_FLEX_WRAP: {
        if (isInitial || isInherit) { style->setFlexWrap(false); return; }
        if (!primitiveValue) return;
        style->setFlexWrap(primitiveValue->getIdent() == CSS_VAL_WRAP);
        break;
    }
    case CSS_PROP_GAP: {
        if (isInitial || isInherit) { style->setGap(0); return; }
        if (!primitiveValue) return;
        if (primitiveValue->primitiveType() == CSSPrimitiveValue::CSS_PX
            || primitiveValue->primitiveType() == CSSPrimitiveValue::CSS_NUMBER) {
            style->setGap(qMax(0, qRound(primitiveValue->floatValue(CSSPrimitiveValue::CSS_PX))));
        }
        break;
    }
