...
bool CSSStyleSheetImpl::parseString(const DOMString &string, bool strict)
{
    ...
    // Strip CSS custom property declarations (--name: value;). KHTML's parser
    // does not support them and a single custom property can cause the whole
    // stylesheet to fail parsing (rules=0). We remove the declarations but
    // keep all other rules intact. This is a generic compatibility fix, not
    // site-specific.
    QString css = string.string();
    {
        QString out;
        ...
        while (i < n) {
            // Detect custom property start: -- followed by name char
            if (i + 1 < n && css.at(i) == QLatin1Char('-') && css.at(i + 1) == QLatin1Char('-')) {
                // Must be at declaration start (after {, ;, or whitespace)
                bool atStart = false;
                for (int j = i - 1; j >= 0; --j) {
                    const QChar c = css.at(j);
                    if (c == QLatin1Char('{') || c == QLatin1Char(';')) { atStart = true; break; }
                    if (!c.isSpace()) break;
                }
                if (atStart) {
                    int k = i + 2;
                    while (k < n && (css.at(k).isLetterOrNumber() || css.at(k) == QLatin1Char('-') || css.at(k) == QLatin1Char('_')))
                        ++k;
                    // Skip whitespace and colon
                    int colon = k;
                    while (colon < n && css.at(colon).isSpace()) ++colon;
                    if (colon < n && css.at(colon) == QLatin1Char(':')) {
                        // Skip value until ; or } at bracket depth 0
                        int depth = 0;
                        int end = colon + 1;
                        QChar quote = QChar::Null;
                        while (end < n) {
                            const QChar c = css.at(end);
                            if (!quote.isNull()) {
                                ...
                                if (depth > 0) --depth;
                                else if (c == QLatin1Char('}')) { break; } // end of block, keep }
    ...
    // Remove empty rule blocks (selector { }) left after stripping custom
    // properties. KHTML parser also fails on empty declaration blocks.
    {
        QRegularExpression reEmpty(
            QStringLiteral("[^{};\\s]+\\s*\\{\\s*\\}"),
            QRegularExpression::CaseInsensitiveOption);
        css = css.remove(reEmpty);
    }

    // Fix malformed CSS where ';' is used instead of '{' at rule-block start.
    // Only when at brace depth 0 (i.e. between rules, not inside a declaration
    // block) and followed by "property:". This avoids touching normal ";".
    {
        QString out;
        out.reserve(css.size());
        int depth = 0;
        ...
        for (int i = 0; i < css.size(); ++i) {
            ...
            if (c == QLatin1Char('{')) { ++depth; out.append(c); continue; }
            if (c == QLatin1Char('}')) { if (depth > 0) --depth; out.append(c); continue; }
            if (depth == 0 && c == QLatin1Char(';')) {
                // Look ahead: whitespace + property-name + ':'
                int j = i + 1;
                while (j < css.size() && css.at(j).isSpace()) ++j;
                int nameStart = j;
                while (j < css.size() && (css.at(j).isLetter() || css.at(j) == QLatin1Char('-'))) ++j;
                if (j > nameStart && j < css.size() && css.at(j) == QLatin1Char(':')) {
    ...
    CSSParser p(strict);
    p.parseSheet(this, DOMString(css));
    return true;
}
...
{
    for (StyleSheetImpl *sh : styleSheets) {
        sh->deref();
    }
}
