...
public:
    ...
    // Out-of-line on purpose: QPointer::data() static_casts its stored
    // QObject to KHTMLPart, which needs the complete type. This header only
    // forward-declares KHTMLPart to stay lightweight, so the definition
    // lives in dom2_eventsimpl.cpp where khtml_part.h is included.
