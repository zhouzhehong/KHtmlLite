...
void DocumentImpl::processHttpEquiv(const DOMString &equiv, const DOMString &content)
{
    ...
    if (strcasecmp(equiv, "refresh") == 0 && v && v->part()->metaRefreshEnabled()) {
        ...
        int pos = QRegularExpression(QStringLiteral("[;,]")).match(str).capturedStart();
        if (pos == -1) {
            pos = QRegularExpression(QStringLiteral("[ \\t]")).match(str).capturedStart();
        }
...
void DocumentImpl::rebuildStyleSheetList(bool force)
{
    ...
    if (m_addedStyleSheets) {
        for (StyleSheetImpl *sh : m_addedStyleSheets->styleSheets) {
    ...
    for (StyleSheetImpl *sh : oldStyleSheets) {
        sh->deref();
    }
}
